...
//
/*****************************************************************
修改 con_check 和 con_save 以及 Mysql_link() 里的link数据
******************************************************************/
#include "stdafx.h"
...
char con_check[65]="G:\\project\\code\\training\\face_match1\\data\\";		//默认的匹配临时目录
char con_save[555] = "G:\\project\\code\\training\\face_match1\\sam\\";	//信息录入时默认的存图片路径
...
void imagefull::ShowImage(IplImage *img_in, UINT ID)
{
	/**********************resize**********************/
		
	CvSize d_size;
	double wscale = 250.0/img_in->width;
	double hscale = 220.0/img_in->height;
	d_size.width = img_in->width * wscale;		//目标图像的宽为源图象宽的scale倍
	d_size.height = img_in->height * hscale;	//目标图像的高为源图象高的scale倍
	IplImage* img = cvCreateImage(d_size,img_in->depth,img_in->nChannels);
	cvResize(img_in,img,CV_INTER_LINEAR);
	
	/**************************************************/
	GetDlgItem(ID)->ShowWindow(FALSE);//关闭窗口显示
	GetDlgItem(ID)->ShowWindow(TRUE);//打开窗口显示，相当于窗体重启，清除Picture控件的残余
...
void imagefull::ShowImage2(IplImage *img_in, UINT ID)
{
	/**********************resize**********************/
...
//	double xxx = imageone->width * wscale;
//	double yyy = imageone->height * hscale;
//		char sss[999];
  //  s//printf(sss,"%f  %f",xxx,yyy);
//	AfxMessageBox(sss);
...
/*	cvNamedWindow( "图像2",   CV_WINDOW_AUTOSIZE );	//创建用于显示目标图像的窗口	
	...
	*/
	...
	ShowImage(dst,IDC_ShowImg2);
	cvReleaseImage(&dst);
}
//清空
void imagefull::clear()
{
	GetDlgItem(IDC_ShowImg1)->ShowWindow(FALSE);//关闭窗口显示
	GetDlgItem(IDC_ShowImg1)->ShowWindow(TRUE);//打开窗口显示，相当于窗体重启，清除Picture控件的残余
	GetDlgItem(IDC_ShowImg2)->ShowWindow(FALSE); 
	GetDlgItem(IDC_ShowImg2)->ShowWindow(TRUE); 
	GetDlgItem(IDC_ShowImg3)->ShowWindow(FALSE); 
	GetDlgItem(IDC_ShowImg3)->ShowWindow(TRUE); 
	m_DataList.ResetContent();//清空人脸信息框
}
...
//	cvDestroyWindow("全景图");
	//清空
	clear();
	cvSaveImage("F:/project/code/training/RELEASEimagestitch0/fullimag.jpg",bb.image33);
}
...
//  float hscale = 0.9;
	...
	*/
	GetDlgItem(IDC_ShowImg2)->ShowWindow(FALSE);//关闭窗口显示
	GetDlgItem(IDC_ShowImg2)->ShowWindow(TRUE);//打开窗口显示，相当于窗体重启，清除Picture控件的残余

	ShowImage(dst1,IDC_ShowImg1);
	cvReleaseImage(&dst1);
...
//            cvNamedWindow("dst",CV_WINDOW_AUTOSIZE);
			...
			if(in_or_check == 1)
            {
				...
				ShowImage(con_dst,IDC_ShowImg2);//在第二个窗体显示检测结果-----这块显示有问题
				OnButton9() ;
			//	AfxMessageBox("ok");
            }
...
//	ShowImg(img);
	if(in_or_check == 3)
	{
		//ShowImg(img);
		//ShowImg(img);
		ShowImage(img,IDC_ShowImg1);//在第1个窗体显示检测红框结果
	}
	//ShowImg(img);
/*	string tt = con_save;
	++num;
...
//	cerr<<path_name<<endl;
	 ...
	 // ShowImg(image);///look_in_here
	  image = RESIZE_PIC(image);///test_in_here
	  //ShowImg(image);
      if( image )
        {
...
void imagefull::MatchWhoPCA(const char*path_who)///can add path or number
{
 ...
 //   char path_who[path_len];
  //  scanf("%s",path_who);
	...
	//ShowImg(pic_to_show);
	ShowImage(pic_to_show,IDC_ShowImg2);//在第2个窗体显示
...
//	  cerr<<pic_name[dex-1]<<endl;
	  ...
	  //ShowImg(img);
	ShowImage2(img,IDC_ShowImg3);//在第3个窗体显示
...
void imagefull::Match()
{
 ...
 //   cerr<<"enter the path of picture to check!"<<endl;
	...
	Draw(tempname,2);
	//添加调用显示人脸信息ShowList(string s)
	/******************gavin chaged 2014/3/12 ************************/
	//ShowList();
	/****************************************************************/
...
//          cout<<"mysql_library_init() failed"<<endl;
   ...
   //       cout<<"mysql_options() failed"<<endl;
	   ...
	   {
		   //这应该返回是连接成功还是失败！
			AfxMessageBox("失败：c++ link mysql fail!");	
	   
	   }
	   else
	   {
			AfxMessageBox("成功：c++ link mysql success!");
	   }
    //          //printf("\nc++ link mysql fail\n");
   //    else
...
void imagefull::getData(string stu_id,string&name,string&course)
{
	name = "哈哈";
	course = "双大";
	MYSQL_RES* result = NULL;
	...
	/******************query for stu_name***********************/
	sql_s = "select stu_name from s_table where stu_id='";
	sql_s += stu_id;
	sql_s += "'";
	SQL_query(sql_s);
	result = mysql_store_result(&mydata);
	...
	/******************query for stu_name***********************/
	/******************query for course_name********************/
	sql_s = "select course_name from course_table where stu_id='";
	sql_s += stu_id;
	sql_s += "'";
	SQL_query(sql_s);
	result = mysql_store_result(&mydata);
	...
	//列表框中输出对应人脸信息
void imagefull::ShowList(string stu_id)
{
	m_DataList.ResetContent();//清空
	/**********************gavin add *****************************/
	string name,course;
	name = "";
	course = "";
	getData(stu_id,name,course);
	/*************************************************************/
	//m_DataList.AddString("学号");
	m_DataList.InsertString(0,"");
/*	m_DataList.AddString("学号: ");
	m_DataList.AddString(" ");
	m_DataList.AddString("姓名: ");
	m_DataList.AddString(" ");
	m_DataList.AddString("课程: ");
	*/
	stu_id = "学号: " + stu_id;
	name = "姓名: " + name;
	course = "课程: " + course;
	m_DataList.InsertString(1,stu_id.c_str());
	m_DataList.InsertString(2," ");
	m_DataList.InsertString(3,name.c_str());
	m_DataList.InsertString(4," ");
	m_DataList.InsertString(5,course.c_str());


}

void imagefull::OnSelchangeList() 
{
	// TODO: Add your control notification handler code here
